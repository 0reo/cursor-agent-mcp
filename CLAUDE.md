...
- **`argv-builder.js`** (pure, no I/O, no spawn) — exports `buildSessionFlags`, `buildWorkspaceFlags`, `buildFinalArgv`, `buildPromptArgv`, `resolveTimeoutMs`, `buildStructuredResult`, `buildJobPollResponse`, `parseModelList`, `resolveSessionRegistryPath`, `upsertSessionEntry`, plus the `DEFAULT_TIMEOUT_MS` constant. Encapsulates every flag-emission rule (model/force/session/workspace), timeout resolution, JSON-result projection, model-list parsing, session-registry upsert algebra, and async job→tool-result translation. Test target for `tests/argv-builder.test.mjs`.
